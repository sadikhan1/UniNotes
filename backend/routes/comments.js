...
router.notes.post('/:id/comments', requireAuth, async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .select('id, content, created_at, user_id, users!fk_comments_user(username)')
    .single()
  ...
  return res.status(201).json({
    ...
    user: { id: data.user_id, username: data.users?.username },
  })
})
...
router.notes.get('/:id/comments', async (req, res) => {
  ...
  return res.status(200).json(
    (data || []).map(c => ({
      ...
      user: { id: c.user_id, username: c.users?.username },
    }))
  )
})
